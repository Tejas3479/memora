...
### Self-Reflection (F47)
Runs weekly per user. Analyzes the user's recent captures to identify:
- Recurring topics and emerging interests.
- Gaps in knowledge coverage.
- Configures Gemini model using `responseMimeType: 'application/json'` to guarantee structurally valid insights and recommendations.

### Consolidation (F48)
Runs periodically. Identifies clusters of semantically similar memories:
- Computes cosine similarities between timeline vectors.
- If similarity is above `0.85`, groups them as near-duplicates.
- Generates a cohesive summary using Gemini.
- Upserts the summarized concept point back to Qdrant, and deletes original duplicate points from Qdrant index to clear redundancy.

### Evaluation (F50)
Runs weekly. Measures the quality of the memory system:
- Samples recent feedback and upvotes/downvotes to compute satisfaction metrics.
- Logs recommendations to optimize query thresholds and fine-tune synthesis prompts.

### Multi-Agent Debate & Stateful Graphs (F24, F33)
Triggered for complex query synthesis and context mapping using **LangGraph**:
- **Agentic Search Graph:** Defines a state workflow (`planSearch`, `executeSearch`, `mergeAndDeduplicate`, and `synthesize`) utilizing type-safe state annotations to resolve complex multi-step queries.
- **Proactive Context Graph:** Analyzes active browser URLs and titles, queries semantic vector space, and ranks nodes to surface contextually relevant memories.

### Dreaming (F51)
Runs during user-configured idle periods. Performs associative jumps through the memory space:
- Compares memory vectors using cosine similarity.
- Identifies items with intermediate semantic connection scores (similarity between `0.65` and `0.82`) representing non-obvious related concepts.
- Generates speculative association cards using Gemini, and caches them in Redis for fast frontend retrieval.
