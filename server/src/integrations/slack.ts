...
import { slackBreaker } from '../lib/circuitBreaker.js';
...
export class SlackIntegration {
    ...
    const response = await slackBreaker.execute(() =>
      retry(
        async () => {
          ...
          if (!res.ok) {
            throw new Error(`Slack OAuth exchange failed with status: ${res.status}`);
          }
          return res;
        },
  ...
  public async syncMessages(accessToken: string, userId: string, since?: Date): Promise<{ messagesProcessed: number }> {
    ...
    try {
      ...
      const channelsRes = await slackBreaker.execute(() =>
        retry(
          async () => {
            ...
            if (!res.ok) {
              throw new Error(`Slack API error conversations.list status ${res.status}`);
            }
            return res;
          },
          { attempts: 3, delay: 1000, backoff: 'exponential' }
      ...
      for (const channel of channels) {
        ...
        try {
          ...
          const historyRes = await slackBreaker.execute(() =>
            retry(
              async () => {
                ...
                if (!res.ok) {
                  throw new Error(`Failed to fetch history for Slack channel ${channel.id}: status ${res.status}`);
                }
                return res;
              },
              { attempts: 3, delay: 1000, backoff: 'exponential' }
          ...
          logger.warn(`Failed to sync channel ${channel.id}:`, channelErr);
        }
      }
