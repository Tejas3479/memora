...
export class SelfReflectionLoop {
  ...
  public async execute(input: SelfReflectionInput): Promise<SelfReflectionOutput> {
    ...
    try {
      ...
      console.warn('[SelfReflectionLoop] Database memory query failed:', err);
    }
        ...
        insights: [
          {
            ...
            description: topMemories.length > 0 
              ? `Identified primary focus around topics related to "${topMemories[0].title}".`
