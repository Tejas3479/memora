...
export class ConsolidationLoop {
    ...
    async execute(input) {
        // 1. Fetch temporal memories
        const { results } = await this.qdrantService.getTimeline(input.userId, 100, 0);
        // 2. Identify near duplicates or clusters based on simple text comparisons
        const duplicates = await this.findDuplicates(results);
        // 3. Synthesize summary for clusters
    ...
    async findDuplicates(memories) {
        ...
        for (let i = 0; i < memories.length; i++) {
            ...
            for (let j = i + 1; j < memories.length; j++) {
                ...
                if (visited.has(b.id))
                    continue;
                // Simple similarity score placeholder (e.g. sharing words)
    ...
    async createSummary(memories) {
        ...
        const response = await model.generateContent(`Summarize the following consolidated documents into one cohesive record:\n\n${content}`);
